...
* `scripts/export_retrievals.py` -- runs in the app's own environment, re-scrapes the CrowdStrike docs, rebuilds the FAISS index with the current chunking config, and for each golden question writes both the retrieved contexts and the `reference_contexts` (the chunks of that question's own source doc, produced with the same splitter + labeling the agent uses at ingestion) to `eval_results/retrievals.json`. It does not score anything -- no RAGAS, no MLflow, no LLM calls happen locally.
* A Databricks notebook reads `retrievals.json` and scores it with RAGAS `NonLLMContextPrecisionWithReference`, logging every run to MLflow so retrieval quality is a trend across pipeline edits, not a single point-in-time number. This metric is deterministic string matching (no judge LLM, no reference answers): each retrieved chunk that came from the correct source doc is byte-identical to one of that question's `reference_contexts`, so the score is the rank-weighted fraction of top-k chunks retrieved from the correct source doc.

The point of the hook is that it's deterministic: the export runs whether or not I remember to run it, because it's wired to the edit itself rather than to my own discipline. Scoring currently requires a manual step in the Databricks notebook -- RAGAS scoring runs in an isolated environment on purpose: `ragas` (0.4.3) declares its `langchain` / `langchain-core` / `langchain-community` dependencies with no upper bound, so installing it alongside the app lets pip resolve the langchain stack up to the 1.x line, which breaks this app's pinned `langchain 0.3.x`. Keeping RAGAS in its own environment avoids destabilizing the working app.

Current baseline (RAGAS `NonLLMContextPrecisionWithReference`): `context_precision ≈ 0.95` over the CrowdStrike golden question set at chunk_size=900, chunk_overlap=200 (13 of 16 top-k chunks retrieved from the correct source doc). Because the metric is deterministic, that value is reproducible from `retrievals.json` alone; the Databricks run logs it to MLflow so the trend across chunking configs is visible. Context recall is scoped as a known next step -- it requires labeling which specific chunk answers each question (the current golden set only maps a question to its source doc, which is enough for source-level precision but not chunk-level recall).
